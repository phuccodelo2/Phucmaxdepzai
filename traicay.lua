...
if _G.FastAttack then
    ...
    local CollectionService = game:GetService("Sưu tầmionService")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    local TeleportService = game:GetService("dịch chuyển Service")
    ...
    Module.FastAttack = (function()
        ...
        function FastAttack:BladeHits()
            ...
            if Equipped and Equipped.ToolTip ~= "súng" then
                self:AttackNearest()
            else
                task.wait(0)
            end
...
if getgenv().Team == "hải quân" then
    ReplicatedStorage.Remotes.CommF_:InvokeServer("chọn phe", "hải quân")
elseif getgenv().Team == "hải tặc" then
    ReplicatedStorage.Remotes.CommF_:InvokeServer("chọn phe", "hải tặc")
end
    ...
    if chooseTeam and chooseTeam.Visible and uiController then
        for _, v in pairs(getgc(true)) do
            if type(v) == "function" and getfenv(v).script == uiController then
                ...
                pcall(function()
                    if (constant[1] == "hải tặc" or constant[1] == "hải quân") and #constant == 1 then
...
-- Rainbow border cho Fluent UI
local RunService = game:GetService("RunService")

-- Tìm container chính của window
local frame = Window.__object or Window.Container or Window
if typeof(frame) == "table" then
    frame = frame.Window or frame.Main or nil
end
...
if frame and frame:IsA("GuiObject") then
    ...
    warn("Không tìm thấy frame gốc của Fluent Window, vui lòng kiểm tra tên container.")
end

local Tabs = {

  info=Window:AddTab({ Title="thông tin ad" }),
...
Main1=Window:AddTab({ Title=" cày khác" }),
    ...
    Misc=Window:AddTab({ Title=" Lặt vặt" }),
}
...
function Tween2(KG)
    ...
    local Speed = 350
    local tween  info = Tween  info.new(Distance / Speed, Enum.EasingStyle.Linear)
    local tween = game:GetService("TweenService"):Create(game.Players.LocalPlayer.Character.HumanoidRootPart, tween  info, {
        CFrame = KG
    })
    tween:Play()
...
Button.Image = "rbxassetid://96179526758636"

-- Bo góc 12 px cho button (mềm mại)
local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 12)  -- thay đổi từ 1 (tròn hẳn) thành 12 pixel bo góc nhẹ
UICorner.Parent = Button

-- Cho phép kéo thả button
...
Tabs.  info:AddButton({
        ...
        Description="Discord",
        Callback=function()
            setclipboard(tostring("https://discord.gg/YBrWARHugC")) 
        end
})
...
Tabs.  info:AddParagraph({
    Title="Update",
    Content="Script hỗ trợ pc và pe"
    
})
Tabs.  info:AddParagraph({
    Title="All Clients Android đều Được hỗ trợ",
    Content=""
})
Tabs.  info:AddParagraph({
    Title="All Clients PC đều Được hỗ trợ",
    Content=""
})
...
local AutoFram = Tabs.Main:AddSection("tự động   cày")
...
local AutoFram = Tabs.Main1:AddSection("CastleRaid")
    local ToggleCastleRaid = Tabs.Main1:AddToggle("ToggleCastleRaid", {
        Title="tự động   Cuộc đột kích lâu đài",
        Description="", 
        Default=false })
...
local ToggleCollectBerry = Tabs.Main1:AddToggle("ToggleCollectBerry", {
    Title = "tự động   nhặt Berry và đổi sever",
    Description = "",
    Default = false
})
    ...
    while wait() do
        if _G.AutoCollectBerry then
            ...
            if Nearest then
                ...
                Fluent:Notify({
                    Title = "PHUCMAX",
                    Content = "Tìm Berry: " .. tostring(BerryName),
                    Duration = 10
                })
            else
                Hop()
...
local Mastery = Tabs.Main:AddSection("tự động   cày thông thạo")
local DropdownMastery = Tabs.Main:AddDropdown("DropdownMastery", {
    Title = "Lựa chọn cày",
    Description = "",
    Values = {"cấp độ", "Cấp độ Không có nhiệm vụ", "Gần Mob", "Xương", "Bánh ngọt", "Ecto"},
    Multi = false,
    Default = 1,
})
...
local ToggleMasteryGun = Tabs.Main:AddToggle("ToggleMasterysúng", {
    Title = "tự động   cày súng",
    Description = "",
    Default = false
})
...
function EquipToolGun()
    pcall(function()
        for i, v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
            if v.ToolTip == "súng" and v:IsA('Tool') then
...
spawn(function()
    ...
    gg.__namecall = newcclosure(function(...)
        ...
        if tostring(method) == "FireServer" then
            if tostring(args[1]) == "Remotesự kiện" then
...
if Sea3 then
local MiscFarm = Tabs.Main:AddSection("cày xương")
...
local ToggleBone = Tabs.Main:AddToggle("Togglexương", {
...
local ToggleRandomBone = Tabs.Main:AddToggle("ToggleRandomxương", {Title="Random xương",Description="", Default=false })
    ...
    while wait() do
        pcall(function()
            ...
            elseif string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner"))==86 then
                ...
                Mob_Kill_Cake_Prince:SetDesc("Cake Prince : có ")
            end
        end)
    end
end)
local ToggleCake = Tabs.Main:AddToggle("ToggleCake", {
    Title = ""tự động  cày Bánh ngọt",
    Description = "", 
    Default = false
})
...
local boss = Tabs.Main:AddSection("cày trùm")
    if Sea1 then
    ...
    local Material = Tabs.Main:AddSection("Danh sách vật liệu")
    if Sea1 then
        MaterialList={
    ...
    local ToggleMaterial = Tabs.Main:AddToggle("ToggleMaterial", {
        Title=""tự động  cày Danh sách vật liệu",
        Description="", 
        Default=false })
...
function UpdateKitsune()
    if game.Workspace._WorldOrigin.Locations:FindFirstChild('Kitsune Island') then
        StatusKitsune:SetDesc("Kitsune Island: Có")
    else
        StatusKitsune:SetDesc("Kitsune Island: Không")
    end
end
spawn(function()
    pcall(function()
...
local ToggleTPKitsune = Tabs.Sea:AddToggle("ToggleTPKitsune", {Title = "dịch chuyển  Kitsune Island", Description = "", Default = false})
...
local ToggleCollectAzure = Tabs.Sea:AddToggle("ToggleCollectAzure", {Title = ""tự động  nhặt Azure", Description = "", Default = false})
...
createToggle("tự động   Find Prehistoric", "tự động  FindPrehistoric", islandsToDelete.Prehistoric, "PrehistoricIsland", "Prehistoric Island")
createToggle("tự động   Find Mirage", "tự động  FindMirage", islandsToDelete.Mirage, "MysticIsland", "Mystic Island")
createToggle("tự động   Find Leviathan", "tự động  FindFrozen", islandsToDelete.Frozen, "FrozenDimension", "Leviathan Island")
local AutoComeTikiToggle = Tabs.Sea:AddToggle("tự động  ComeTiki", {
    Title=""tự động  Tiki Island",
    Description="",
    Default=false
})
...
local AutoComeHydraToggle = Tabs.Sea:AddToggle("tự động  ComeHydra", {
    Title = ""tự động  tới đảo Hydra",
    Description="",
    Default=false
})
...
spawn(function()
    ...
    local ToggleShip = Tabs.Sea:AddToggle("ToggleShip", {Title="tự động  Ship",Description="", Default=false })
        ...
        while wait() do
            pcall(function()
                if game:GetService("ReplicatedStorage"):FindFirstChild("Diablo") or game:GetService("ReplicatedStorage"):FindFirstChild("Deandre") or game:GetService("ReplicatedStorage"):FindFirstChild("Urban") or game:GetService("Workspace").Enemies:FindFirstChild("Diablo") or game:GetService("Workspace").Enemies:FindFirstChild("Deandre") or game:GetService("Workspace").Enemies:FindFirstChild("Urban") then
                    StatusElite:SetDesc("Elite Boss: có  | Killed:  "..game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("EliteHunter","Progress"))    
                else
                    StatusElite:SetDesc("Elite Boss: không  | Killed: "..game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("EliteHunter","Progress"))    
                end
            end)
        end
    end)
    local ToggleElite = Tabs.Main:AddToggle("ToggleElite", {Title="tự động   Fram Elite",Description="", Default=false })
...
if Sea3 then
    ...
    Tabs.Sea:AddButton({
        Title="dịch chuyển  To HighestPoint",
        Description="",
        Callback=function()
            TweenToHighestPoint()
        end
...
local ToggleTpAdvanced = Tabs.Sea:AddToggle("ToggleTpAdvanced", {
    Title="dịch chuyển  To Advanced Fruit Dealer", 
    Description="", 
    Default=false
})
...
local ToggleTweenGear = Tabs.Sea:AddToggle("ToggleTweenGear", {Title="dịch chuyển  To Gear",Description="", Default=false })
...
if Sea3 then
    local ToggleHallow = Tabs.Item:AddToggle("ToggleHallow", {Title="tự động   Hallow",Description="", Default=false })
    ...
    spawn(function()
           ...
           local ToggleYama = Tabs.Main1:AddToggle("ToggleYama", {Title="tự động   Yama",Description="", Default=false })
        ...
        local ToggleTushita = Tabs.Main1:AddToggle("ToggleTushita", {Title="tự động   Tushita",Description="", Default=false })
        ...
        Options.ToggleTushita:SetValue(false)
           spawn(function()
                   ...
                   local ToggleHoly = Tabs.Main1:AddToggle("ToggleHoly", {Title="tự động   Holy",Description="", Default=false })
            ...
            if _G.Auto_ObservationV2 then
                if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CitizenQuestProgress", "Citizen")==3 then
                    ...
                    elseif game:GetService("Players").LocalPlayer.Backpack:FindFirstChild("Fruit Bowl") 
                        ...
                        wait(1)
                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("KenTalk2", "mua")
                    else
                        ...
                        if (CFrame.new(-9681.458984375, 6.139880657196045, 6341.3720703125).Position-game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude<=5000 then
                            if game:GetService("Workspace").NPCs:FindFirstChild("Skeleton Machine") then
                                ...
                                if game:GetService("Workspace").Map["Haunted Castle"].Candle1.Transparency==0 then
                                    ...
                                    if string.find(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("gravestonesự kiện",2), "Error") then
                                        Tween(CFrame.new(-8653.2060546875, 140.98487854003906, 6160.033203125))
                                    elseif string.find(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("gravestonesự kiện",2), "Nothing") then
                                        Tween("Wait Full Moon")
                                    else
                                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("gravestonesự kiện",2,true)
                                    end
                                end
                            end
                        else
                ...
                pcall(function()
                    ...
                    elseif game:GetService("Workspace").Map:FindFirstChild("HellDimension") then
                        ...
                        if game:GetService("Workspace").Enemies:FindFirstChild("Soul Reaper") or game.ReplicatedStorage:FindFirstChild("Soul Reaper [Lv. 2100] [Raid Boss]") then
                            ...
                            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Bones","mua",1,1)
                        end
                    end
                end)
            end
...
if Sea2 then
        local ToggleFactory = Tabs.Main1:AddToggle("ToggleFactory", {Title="tự động   Factory",Description="", Default=false })
...
if Sea2 or Sea3 then
    local ToggleHakiColor = Tabs.Shop:AddToggle("ToggleHakiColor", {Title="tự động   Collect Haki",Description="",Default=false })
...
if Sea2 then
    local ToggleSwordLengend = Tabs.Shop:AddToggle("ToggleSwordLengend", {Title="tự động   Sword Legend",Description="",Default=false })
...
if Sea2 then
    local ToggleEvoRace = Tabs.Main1:AddToggle("ToggleEvoRace", {Title="tự động   Race V2", Description="", Default=false})
...
local ToggleAutoT = Tabs.Setting:AddToggle("ToggleAutoT", {Title="tự động   On V3", Description="", Default=false })
...
local ToggleAutoY = Tabs.Setting:AddToggle("ToggleAutoY", {Title="tự động   On V4", Description="", Default=false })
...
local Input = Tabs.Status:AddInput("Input", {
    ...
    Tabs.Status:AddButton({
        ...
        Description="",
        Callback=function()
            game:GetService("dịch chuyển Service"):TeleportToPlaceInstance(game.placeId,_G.Job, game.Players.LocalPlayer)
        end
    })
    Tabs.Status:AddButton({
        Title="Copy Job ID",
...
local ToggleMelee = Tabs.Stats:AddToggle("Togglecận chiến", {Title="Add cận chiến",Description="", Default=false })
...
local ToggleSword = Tabs.Stats:AddToggle("ToggleKiếm", {Title="Add Kiếm",Description="", Default=false })
ToggleSword:OnChanged(function(Value)
    _G.Auto_Stats_Sword=Value
    end)
Options.ToggleSword:SetValue(false)
local ToggleGun = Tabs.Stats:AddToggle("Togglesúng", {Title="Add súng", Description="",Default=false })
...
local ToggleTeleport = Tabs.Player:AddToggle("ToggleTeleport", {Title="dịch chuyển  Player", Description="",Default=false })
    ...
    pcall(function()
        while wait() do
            if _G.Random_Auto then
                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Cousin","mua")
            end 
        end
    end)
end)
local ToggleCollectTP = Tabs.Fruit:AddToggle("ToggleCollectTP", {Title="dịch chuyển  Fruit",Description="", Default=false })
...
local ToggleCollect = Tabs.Fruit:AddToggle("ToggleCollect", {Title="Sưu tầm Fruit",Description="", Default=false })
...
local ToggleBuy = Tabs.Raid:AddToggle("ToggleBuy", {Title="muaChip", Description="",Default=false })
...
local ToggleGetFruit = Tabs.Raid:AddToggle("ToggleGetFruit", {Title="Sưu tầm Fruit 1M",Description="", Default=false })
...
local Mastery = Tabs.Raid:AddSection("dịch chuyển  Raid Law")
...
local ToggleHumanandghoul = Tabs.Race:AddToggle("ToggleHumanandghoul", {Title="tự động   Trial Human/Ghoul",Description="", Default=false })
ToggleHumanandghoul:OnChanged(function(Value)
    KillAura=Value
end)
Options.ToggleHumanandghoul:SetValue(false)
local ToggleAutotrial = Tabs.Race:AddToggle("ToggleAutotrial", {Title="tự động   Trial",Description="", Default=false })
...
spawn(function()
    while wait() do
        if AutoFarmRace then 
            pcall(function()
                if game.Players.LocalPlayer.Character:FindFirstChild("tộcTransformed") then
...
Tabs.Shop:AddButton({
    ...
    Description="",
    Callback=function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("KenTalk","mua")
    end
})
...
local Mastery = Tabs.Shop:AddSection("cận chiến")
Tabs.Shop:AddButton({
    Title="Black Leg",
    Description="",
    Callback=function()
...
Tabs.Misc:AddButton({
    ...
    Description="",
    Callback=function()
        game:GetService("dịch chuyển Service"):Teleport(game.PlaceId, game:GetService("Players").LocalPlayer)
    end
})
Tabs.Misc:AddButton({
        ...
        for i,v in pairs(Site.data) do
            ...
            if tonumber(v.maxPlayers)>tonumber(v.playing) then
                ...
                if Possible==true then
                    ...
                    pcall(function()
                        wait()
                        game:GetService("dịch chuyển Service"):TeleportToPlaceInstance(PlaceID, ID, game.Players.LocalPlayer)
                    end)
                    wait()
                end
            end
...
Tabs.Misc:AddButton({
    ...
    Description="",
    Callback=function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("chọn phe","hải tặc") 
    end
})
Tabs.Misc:AddButton({
    Title="hải quân",
    Description="",
    Callback=function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("chọn phe","hải quân") 
    end
})
        ...
        if _G.AutoRejoin then
                getgenv().rejoin=game:GetService("CoreGui").RobloxPromptGui.promptOverlay.ChildAdded:Connect(function(child)
                    if child.Name=='ErrorPrompt' and child:FindFirstChild('MessageArea') and child.MessageArea:FindFirstChild("ErrorFrame") then
                        game:GetService("dịch chuyển Service"):Teleport(game.PlaceId)
                    end
                 end)
            end
        end
        ...
        if _G.AutoHydraTree then
            ...
            for _, target in ipairs(targets) do
                ...
                if character and character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if distance <= 1 then
                        equipAndUseSkill("cận chiến")
                        equipAndUseSkill("Kiếm")
                        equipAndUseSkill("súng")
                    end
                end
            end
        end
...
Tabs.Main1:AddButton({
    ...
    Description = "Yêu cầu 15 Tàn Lửa Blaze + 10 Sắt",
    Callback=function()
        local args = {
            [1]="CraftItem",
            [2]="chế tạo",
...
Tabs.Main1:AddButton({
    Title = "Chế tạo Mũ Khủng Long",
    Description = "Yêu cầu 25 Xương Khủng Long + 10 Ngà Nhỏ",
    Callback = function()
        local args = {
            [1] = "CraftItem",
            [2] = "chế tạo",
...
Tabs.Main1:AddButton({
    Title = "Chế tạo Sọ T-Rex",
    Description = "Yêu cầu 8 Xương Khủng Long + 5 Vảy Rồng",
    Callback = function()
        local args = {
            [1] = "CraftItem",
            [2] = "chế tạo",
    ...
    pcall(function()
        while wait() do
            if game:GetService("Workspace").Map:FindFirstChild("PrehistoricIsland") then
                Prehistoric:SetDesc("Volcano Island: có ")
            else
                Prehistoric:SetDesc("Volcano Island: không ")
            end
        end
    end)
end)
local ToggleTPVolcano = Tabs.Sea:AddToggle("ToggleTPVolcano", { 
    Title="dịch chuyển  Volcano Island", 
    Description="", 
    Default=false 
})
...
local ToggleMelee = Tabs.Sea:AddToggle("Togglecận chiến", {
    Title = "Use cận chiến", 
    Description = "", 
    Default = false
})
...
local ToggleSword = Tabs.Sea:AddToggle("ToggleKiếm", {
    Title = "Use Kiếm", 
    Description = "", 
    Default = false
})
...
local ToggleGun = Tabs.Sea:AddToggle("Togglesúng", {
    Title = "Use súng", 
    Description = "", 
    Default = false
})
...
local ToggleKillAura = Tabs.Sea:AddToggle("ToggleKillAura", {Title="tự động  giết Golems",Description="", Default=false })
...
local ToggleCollectBone = Tabs.Sea:AddToggle("ToggleCollectxương", {
    Title="Sưu tầm xương", 
    Description="", 
    Default=false
})
...
local ToggleCollectEgg = Tabs.Sea:AddToggle("ToggleCollecttrứng", {
    Title = "Sưu tầm trứng",
    Description = "",
    Default = false
})
