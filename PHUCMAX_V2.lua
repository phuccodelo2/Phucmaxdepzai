...
cardFrame.Name = "PHUCMAXCard"
cardFrame.Size = UDim2.new(0, 430, 0, 320)
cardFrame.Position = UDim2.new(0.5, -215, 0.5, -160)
cardFrame.BackgroundColor3 = Color3.fromRGB(12, 14, 24)
cardFrame.BackgroundTransparency = 0.18
cardFrame.Parent = containerFrame
...
cardStroke.Color = Color3.fromRGB(110, 160, 255)
cardStroke.Parent = cardFrame

local cardGradient = Instance.new("UIGradient")
cardGradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, Color3.fromRGB(26, 30, 52)),
...
glow.Size = UDim2.new(0, 280, 0, 280)
glow.Position = UDim2.new(0.5, -140, 0.5, -150)
glow.Parent = cardFrame

-- Hình ảnh ở giữa màn hình
local imageLabel = Instance.new("ImageLabel")
imageLabel.Size = UDim2.new(0, 170, 0, 170)
imageLabel.Position = UDim2.new(0.5, -85, 0.5, -95)
...
timeLabel.Size = UDim2.new(0, 260, 0, 24)
timeLabel.Position = UDim2.new(0.5, -110, 0.5, 120)
...
lixuryxLabel.Position = UDim2.new(0.5, -150, 0.5, -142)
...
lixuryxLabel.Text = "PHUCMAX | Auto Bounty"
lixuryxLabel.Parent = containerFrame

-- Label nhỏ nằm trên: (free)
local premiumLabel = Instance.new("TextLabel")
premiumLabel.Size = UDim2.new(0, 90, 0, 15)
premiumLabel.Position = UDim2.new(0.5, 115, 0.5, -140) -- lệch lên và sang phải
...
bountyLabel.Size = UDim2.new(0, 230, 0, 22)
bountyLabel.Position = UDim2.new(0.5, -115, 0.5, -108)
...
bountyLabel.Text = "Current Bounty: " .. tostring((game:GetService("Players").LocalPlayer:FindFirstChild("leaderstats") and game:GetService("Players").LocalPlayer.leaderstats:FindFirstChild("Bounty/Honor") and game:GetService("Players").LocalPlayer.leaderstats["Bounty/Honor"].Value) or 0)
...
PHUCMAX.Theme = {
    ...
    Accent2 = Color3.fromRGB(156, 103, 255),
...
statsPanel.Name = "PHUCMAXStatsPanel"
statsPanel.Size = UDim2.new(0, 240, 0, 132)
statsPanel.Position = UDim2.new(1, -260, 0, 56)
statsPanel.BackgroundColor3 = Color3.fromRGB(13, 15, 26)
statsPanel.BackgroundTransparency = 0.15
statsPanel.Parent = containerFrame
...
statsStroke.Color = PHUCMAX.Theme.Accent
statsStroke.Transparency = 0.2
...
statsGradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, Color3.fromRGB(21, 26, 44)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(10, 12, 18))
}
...
panelTitle.Size = UDim2.new(1, -22, 0, 22)
panelTitle.Position = UDim2.new(0, 12, 0, 10)
panelTitle.Font = Enum.Font.GothamBold
panelTitle.Text = "PHUCMAX V2 // STATUS"
panelTitle.TextXAlignment = Enum.TextXAlignment.Left
panelTitle.TextColor3 = PHUCMAX.Theme.Text
panelTitle.TextSize = 14
panelTitle.Parent = statsPanel

local function makeStatLine(order, prefix)
    local lb = Instance.new("TextLabel")
    lb.BackgroundTransparency = 1
    lb.Size = UDim2.new(1, -24, 0, 20)
    lb.Position = UDim2.new(0, 12, 0, 16 + (order * 24))
    lb.Font = Enum.Font.GothamSemibold
    lb.TextXAlignment = Enum.TextXAlignment.Left
    lb.TextColor3 = PHUCMAX.Theme.Muted
    lb.TextSize = 13
    lb.Text = prefix
    lb.Parent = statsPanel
    return lb
end

local statusLine = makeStatLine(1, "Status: Booting")
local targetLine = makeStatLine(2, "Target: None")
local actionLine = makeStatLine(3, "Action: Idle")
local errorLine = makeStatLine(4, "Error: None")

local function refreshStatsPanel()
    local err = PHUCMAX.LastError ~= "" and PHUCMAX.LastError or "None"
    if #err > 32 then
        err = err:sub(1, 29) .. "..."
    end
    statusLine.Text = "Status: " .. tostring(PHUCMAX.Status)
    targetLine.Text = "Target: " .. tostring(PHUCMAX.TargetName)
    actionLine.Text = "Action: " .. tostring(PHUCMAX.LastAction)
...
pxLog("PHUCMAX V2 core loaded", "BOOT")
...
function target() 
    pxSafeCall("target", function()
        pxSetStatus("Searching", "Scanning players")
        ...
        for _, v in ipairs(game.Players:GetPlayers()) do
            ...
            if validTeam and v ~= lp and not hasValue(getgenv().checked, v) then
                ...
                if getgenv().Setting.Skip.Fruit then
                    fruitOk = data and data:FindFirstChild("DevilFruit") and not hasValue(getgenv().Setting.Skip.FruitList, data.DevilFruit.Value)
                end

                if data and root and hum and hum.Health > 0 and fruitOk and root.Position.Y <= 12000 and bountyStat and not hopserver then
                    local distance = (root.Position - myRoot.Position).Magnitude
                    local levelStat = data:FindFirstChild("Level")
                    local levelOk = levelStat and (tonumber(lp.Data.Level.Value) - 250) < levelStat.Value
                    ...
                    if levelOk and bountyOk and v4Ok and distance < closestDistance then
                        picked = v
        ...
        if picked then
            ...
            PHUCMAX.TargetName = "None"
            pxLog("Không có mục tiêu hợp lệ, chuyển server", "WARN")
            HopServer()
        end
    end)
end
...
local function tryUseSkill(skillRoot, keyName, cfg)
    ...
    if slot and slot:FindFirstChild("Cooldown") and slot.Cooldown.AbsoluteSize.X <= 0 then
...
local function chooseWeapon()
    if getgenv().Setting.Fruit.Enable then
        return "Blox Fruit", getgenv().Setting.Fruit
    elseif getgenv().Setting.Sword.Enable then
        return "Sword", getgenv().Setting.Sword
    elseif getgenv().Setting.Melee.Enable then
        return "Melee", getgenv().Setting.Melee
    elseif getgenv().Setting.Gun.Enable then
        return "Gun", getgenv().Setting.Gun
    end
    return "Melee", getgenv().Setting.Melee
end

local function useCurrentWeapon()
    local tooltip, cfg = chooseWeapon()
    getgenv().weapon = tooltip
    pxSetStatus("Combat", "Using " .. tooltip)
    ...
    if tooltip == "Blox Fruit" then
        acted = tryUseSkill(skillRoot, "Z", cfg.Z) or
                tryUseSkill(skillRoot, "X", cfg.X) or
                tryUseSkill(skillRoot, "C", cfg.C) or
                tryUseSkill(skillRoot, "V", cfg.V) or
                tryUseSkill(skillRoot, "F", cfg.F)
    elseif tooltip == "Sword" then
        acted = tryUseSkill(skillRoot, "Z", cfg.Z) or
                tryUseSkill(skillRoot, "X", cfg.X)
    elseif tooltip == "Gun" then
        ...
        acted = tryUseSkill(skillRoot, "Z", cfg.Z) or
                tryUseSkill(skillRoot, "X", cfg.X) or
                tryUseSkill(skillRoot, "C", cfg.C)
    end

    if not acted and getgenv().Setting.Click.AlwaysClick then
        Click()
    end
end
    ...
    while task.wait(1) do
        pxSafeCall("ui monitor", function()
            ...
            if ls and ls:FindFirstChild("Bounty/Honor") then
                bountyValue = ls["Bounty/Honor"].Value
            end
            bountyLabel.Text = "Current Bounty: " .. tostring(bountyValue)
            playerNameLabel.Text = "(" .. lp.Name .. ")"
...
footerBar.Size = UDim2.new(0, 430, 0, 8)
footerBar.Position = UDim2.new(0.5, -215, 0.5, 150)
footerBar.BackgroundColor3 = Color3.fromRGB(20, 24, 34)
footerBar.BorderSizePixel = 0
footerBar.Parent = containerFrame

local footerCorner = Instance.new("UICorner")
footerCorner.CornerRadius = UDim.new(0, 99)
footerCorner.Parent = footerBar

local footerFill = Instance.new("Frame")
footerFill.Name = "Fill"
footerFill.Size = UDim2.new(0.22, 0, 1, 0)
footerFill.BackgroundColor3 = PHUCMAX.Theme.Accent
footerFill.BorderSizePixel = 0
footerFill.Parent = footerBar

local footerFillCorner = Instance.new("UICorner")
footerFillCorner.CornerRadius = UDim.new(0, 99)
footerFillCorner.Parent = footerFill

local footerGlow = Instance.new("UIGradient")
footerGlow.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, PHUCMAX.Theme.Accent),
    ColorSequenceKeypoint.new(1, PHUCMAX.Theme.Accent2)
}
footerGlow.Parent = footerFill

spawn(function()
    while task.wait(0.2) do
        local alpha = (math.sin(tick() * 1.6) + 1) / 2
        footerFill.Size = UDim2.new(0.18 + (alpha * 0.64), 0, 1, 0)
    end
end)
...
-- === PHUCMAX V2 NOTES ===
-- V2 bổ sung panel trạng thái, log vòng đời, wrapper bắt lỗi, click up/down an toàn,
-- target selection chặt hơn, hook aim có kiểm tra executor support và đồng bộ UI runtime.
-- Mục tiêu của bản này là tăng độ ổn định khi respawn / đổi mục tiêu / lỗi thiếu remote.
