...
local function StopAll()
    ...
    if game.CoreGui:FindFirstChild("PHUCMAX_UI") then
        game.CoreGui.PHUCMAX_UI:Destroy()
    end
    warn("⛔ Script đã STOP")
end
...
function MainLoop()
    while _G.ScriptRunning do
        ...
        if _G.AutoFruit then
            for _,fruit in pairs(workspace:GetChildren()) do
                ...
                if fruit:IsA("Tool") and fruit:FindFirstChild("Handle") then
                    ...
                    if hrp then
                        hrp.CFrame = CFrame.new(fruit.Handle.Position + Vector3.new(0,5,0))
                    end
                    task.wait(1)
                end
            end
        ...
        if not found and _G.AutoHop then
            -- chờ 4s rồi hop
            local t = 0
            while t < 10 and _G.ScriptRunning do
                task.wait(1)
                t = t + 1
            end
...
-- // NÚT STOP/START
if game.CoreGui:FindFirstChild("PHUCMAX_STOPBTN") then
    game.CoreGui.PHUCMAX_STOPBTN:Destroy()
end

local sg = Instance.new("ScreenGui", game.CoreGui)
sg.Name = "PHUCMAX_STOPBTN"

local btn = Instance.new("TextButton", sg)
btn.Size = UDim2.new(0,140,0,40)
btn.Position = UDim2.new(0,20,0.4,0) -- cao hơn giữa
btn.Text = "⛔ STOP"
...
task.spawn(function()
    while task.wait(1) do
        ...
        if not found and _G.AutoHop then
            task.wait(4) -- chờ 4s rồi hop server
            if _G.ScriptRunning then
                serverHop()
            end
        end
